...
open class OAuth2Securable: OAuth2Requestable {
	...
	/// If set to `true` (the default) will use system keychain to store tokens.
    /// Use `"keychain": bool` in settings.
	...
	/// The keychain account to use to store tokens.
    /// Defaults to "currentTokens".
	...
	/// The keychain account name to use to store client credentials.
    /// Defaults to "clientCredentials".
    ...
    /// MUST be set via `keychain_access_group` init setting.
	open internal(set) var keychainAccessMode = kSecAttrAccessibleWhenUnlocked

	/// Keychain access group, none is set by default.
    /// MUST be set via `keychain_access_group` init setting.
	open internal(set) var keychainAccessGroup: String?

    /// Base initializer.
    ///
    /// Looks at the `verbose`, `keychain`, `keychain_access_mode`, `keychain_access_group`
    /// `keychain_account_for_client_credentials` and `keychain_account_for_tokens`.
    /// Everything else is handled by subclasses.
    /// - Parameter settings: OAuth settings in `JSON` value.
	...
	/// The service key under which to store keychain items.
    /// Returns "http://localhost", subclasses override to return the authorize `URL`.
	...
	private func updateFromKeychain() {
		...
		do {
            let creds = OAuth2KeychainAccount(oauth2: self, account: keychainAccountForClientCredentials)
			let creds_data = try creds.saved()
            creds_data.flatMap(updateFromKeychainItems)
		} catch {
			logger?.warn("OAuth2", msg: "Failed to load client credentials from keychain: \(error)")
		}

		do {
            let toks = OAuth2KeychainAccount(oauth2: self, account: keychainAccountForTokens)
			let toks_data = try toks.saved()
            toks_data.flatMap(updateFromKeychainItems)
		} catch {
	...
	func updateFromKeychainItems(_ items: [String: String]) {
	}

    /// Items that should be stored when storing client credentials.
    /// - Returns: A dictionary with `String` keys and `String` items.
	open func storableCredentialItems() -> [String: String]? {
		nil
	}
	...
	open func storeClientToKeychain() {
		if let items = storableCredentialItems() {
			...
			let keychain = OAuth2KeychainAccount(
                oauth2: self,
                account: keychainAccountForClientCredentials
            )
			do {
				try keychain.save(items)
			} catch {
    ...
    /// - Returns: A dictionary with `String` keys and `String` items
	open func storableTokenItems() -> [String: String]? {
		nil
	}
