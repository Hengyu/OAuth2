...
open class OAuth2Base: OAuth2Securable {
	...
	*/
	...
	override open func updateFromKeychainItems(_ items: [String: String]) {
	...
	override open func storableCredentialItems() -> [String: String]? {
		clientConfig.storableCredentialItems()
	}

	override open func storableTokenItems() -> [String: String]? {
    ...
    /// Return an OAuth2Request, a `NSMutableURLRequest` subclass, that has already been signed and
    /// can be used against your OAuth2 endpoint.
    ///
    /// This method by default ignores locally cached data and specifies a timeout interval of 20 seconds.
    /// This should be ideal for small `JSON` data requests, but you probably don't want to disable cache for binary
    /// data like avatars.
    /// - Parameters:
    ///   - url: The `URL` to create a request for
    ///   - cachePolicy: The cache policy to use, defaults to
    ...
    ///
    /// The method is only made public in case you want to create a subclass and call ``didAuthorize(withParameters:)``
    /// at an override point. If you call this method yourself on your OAuth2 instance you might screw things up badly.
    ...
    ///
    /// This method is only made public in case you want to create a subclass and need to call ``didFail(error:)``
    /// at an override point. If you call this method yourself on your OAuth2 instance you might screw things up royally.
    /// - Parameter error: The error that led to authorization failure; will use ``OAuth2Error/requestCancelled``
