        ...
        if #available(iOS 11, *), config.ui.useAuthenticationSession {
            ...
            if #available(iOS 9, *), config.ui.useSafariView {
                ...
                if config.authorizeEmbeddedAutoDismiss {
                    oauth2.internalAfterAuthorizeOrFail = { _, _ in
        ...
        let completionHandler: (URL?, Error?) -> Void = { url, error in
            if let url = url {
                ...
                if #available(iOS 12.0, *),
                   let authenticationSessionError = error as? ASWebAuthenticationSessionError {
                    switch authenticationSessionError.code {
                    case .canceledLogin:
                        self.oauth2.didFail(with: .requestCancelled)
                    default:
                        self.oauth2.didFail(with: authenticationSessionError.asOAuth2Error)
                    }
                } else if let sfError = error as? SFAuthenticationError {
                    switch sfError.code {
                    case .canceledLogin:
                        self.oauth2.didFail(with: .requestCancelled)
                    default:
                        self.oauth2.didFail(with: sfError.asOAuth2Error)
                    }
                } else {
