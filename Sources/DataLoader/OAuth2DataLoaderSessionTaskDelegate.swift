...
/// Simple implementation of a session task delegate, which looks at HTTP redirecting,
/// approving redirects in the same domain and re-signing the redirected request.
open class OAuth2DataLoaderSessionTaskDelegate: NSObject, URLSessionTaskDelegate {
    ...
    /// - Parameters:
    ///   - loader: The data loader for which the receiver is delegating.
    ///   - host: The host on which HTTP redirecting will be approved;
    ///   will be run through `URLComponents` to satisfy formatting.
	...
	open func urlSession(
		...
		guard request.url?.host == host else {
			loader?.logger?.warn(
                "OAuth2",
                msg: "Redirected to «\(request.url?.host ?? "nil")» " +
                "but only approving HTTP redirection on «\(host)», " +
                "not following redirect: \(request)"
            )
		...
		do {
			...
			loader.logger?.debug(
                "OAuth2",
                msg: "Following HTTP redirection to «\(request.url?.description ?? "nil")»"
            )
			completionHandler(newRequest)
		} catch {
			loader?.logger?.warn(
                "OAuth2",
                msg: "Failed to re-sign request after HTTP redirection: \(error)"
            )
